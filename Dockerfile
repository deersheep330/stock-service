...
CMD /usr/local/bin/python /home/app/symbol_entry.py && gunicorn -k uvicorn.workers.UvicornWorker fastapi_entry:app -D && /bin/bash ./cron_entrypoint.sh > /proc/1/fd/1 2>/proc/1/fd/2
# CMD ["cron", "-f"]
